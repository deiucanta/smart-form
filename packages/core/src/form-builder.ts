...
export type FieldDef = TextDef | TextareaDef | SelectDef | ArrayDef;
...
class FormBuilder<Fields = EmptyFields> implements FormDef<Fields> {
  ...
  toZod() {
    return this._buildZodSchema(
      this._fields
    ) as z.ZodObject<{ [K in keyof Fields]: z.ZodType<Fields[K]> }>;
  }
...
export type InferFormData<T extends FormDef> = T extends FormDef<infer F>
  ? F
  : never;
